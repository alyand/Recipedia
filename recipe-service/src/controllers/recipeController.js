...
const { ROUTING_KEYS } = require('Recipedia\shared\rabbitmq\events.config.js');
...
exports.getAllRecipes = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Gagal mengambil resep', detail: err.message });
  }
};
...
exports.getRecipesByUserId = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Gagal mengambil resep berdasarkan user ID', detail: err.message });
  }
};
...
exports.createRecipe = async (req, res) => {
  ...
  try {
    ...
    const newRecipe = new Recipe({ userId, title, description, ingredients, steps });
...
exports.updateRecipe = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Gagal memperbarui resep', detail: err.message });
  }
};
...
exports.deleteRecipe = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Gagal menghapus resep', detail: err.message });
  }
};